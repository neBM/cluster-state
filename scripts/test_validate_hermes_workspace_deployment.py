...
EXPECTED_PVC = {
    ...
    "spec": {
        ...
        "storageClassName": "windsor-nfs-rwx",
...
EXPECTED_FLUX_DEPENDENCIES = [{"name": name} for name in ("storage", "platform", "shared-services", "storage-nfs-csi")]
...
def validate_flux_dependencies(value: Any, label: str) -> None:
    if not strict_equal(value, EXPECTED_FLUX_DEPENDENCIES):
        fail(f"{label}: Flux apps dependencies differ from the exact contract")

def governed(item: dict[str, Any]) -> bool:
    metadata = item.get("metadata") or {}
    if metadata.get("namespace") != NAMESPACE:
        return False
    if metadata.get("name") in {WORKSPACE, PVC, SECRET}:
        return True
...
def validate_repository(root: Path) -> None:
    ...
    flux = load_object(root / "clusters/k3s-homelab/flux-system/kustomization-apps.yaml")
    validate_flux_dependencies((flux.get("spec") or {}).get("dependsOn"), "source")
    ...
    if OLD_IMAGE in source_text:
        fail("old amd64-only image digest is present")
...
def run_mutations() -> None:
    ...
    cases: list[tuple[str, Callable[[list[dict[str, Any]]], None]]] = [
        ...
        ("old PVC name", lambda docs: pvc(docs)["metadata"].update(name=WORKSPACE)),
        ("old PVC class", lambda docs: pvc(docs)["spec"].update(storageClassName="local-path-retain")),
        ("old PVC access", lambda docs: pvc(docs)["spec"].update(accessModes=["ReadWriteOnce"])),
        ("old PVC claim", lambda docs: deploy(docs)["volumes"][0]["persistentVolumeClaim"].update(claimName=WORKSPACE)),
    ...
    validate_flux_dependencies(EXPECTED_FLUX_DEPENDENCIES, "dependency fixture")
    try:
        validate_flux_dependencies(EXPECTED_FLUX_DEPENDENCIES[:-1], "missing storage-nfs-csi")
    except AssertionError: print("PASS: missing storage-nfs-csi rejected")
    else: fail("missing storage-nfs-csi: mutation escaped validation")
