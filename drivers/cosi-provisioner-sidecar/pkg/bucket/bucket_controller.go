...
func (b *BucketListener) Add(ctx context.Context, inputBucket *v1alpha1.Bucket) error {
	...
	if bucket.Status.BucketReady {
		...
		return b.updateBucketClaimStatus(ctx, bucket)
	}
...
func (b *BucketListener) updateBucketClaimStatus(ctx context.Context, bucket *v1alpha1.Bucket) error {
	...
	if bucketClaim.Status.BucketName != "" && bucketClaim.Status.BucketName != bucket.Name {
		return errors.Errorf("bucketClaim %s/%s is bound to bucket %s, expected %s",
			bucketClaim.Namespace, bucketClaim.Name, bucketClaim.Status.BucketName, bucket.Name)
	}
