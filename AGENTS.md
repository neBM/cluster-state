...
- **Flux storage prune policy**: `storage-classes` and `storage-access` use `prune: true`; `storage` and `storage-backups` use `prune: false`. Keep the SeaweedFS runtime non-pruning because it owns the live backend, the data-bearing pods use retained host paths, and accidental manifest deletion is more likely to cause cluster-wide storage outage than direct data loss. If you need more pruning in this area, split the runtime further first rather than flipping it wholesale.
...
- Restic backs up restore-critical SeaweedFS PVCs through explicit read-only PVC mounts (`restic-critical-pvc-backup`) and runs repository prune/check separately (`restic-repo-maintenance`). The legacy whole-root `/buckets` CronJob/PV/PVC has been removed; do not recreate scheduled filer-root scans.
...
| restic-critical-pvc-backup | CronJob | Scoped restore-critical SeaweedFS PVC backup (daily 3am); legacy whole-root `restic-backup` removed |
