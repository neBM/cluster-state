...
- **Flux `storage` prune policy**: Keep `clusters/k3s-homelab/flux-system/kustomization-storage.yaml` at `prune: false`. That layer owns the live SeaweedFS backend, the `seaweedfs` and `local-path-retain` `StorageClass` objects, and the restic backup PV/PVC path. Accidental pruning is more likely to cause cluster-wide storage outage than direct data deletion because the data-bearing pods use retained host paths and the managed PVs use `Retain`. Split the storage layer into smaller Flux `Kustomization` objects before enabling pruning there.
