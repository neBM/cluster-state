...
| `overseerr-litestream` | Active rollback source | COSI `BucketAccess/default/overseerr-litestream` creates `overseerr-litestream-s3`; `apps/seerr/deployment-default-seerr.yaml` mounts `BucketInfo` as the legacy fallback restore source |
| `seerr-litestream` | Active | COSI `BucketAccess/default/seerr-litestream` creates `seerr-litestream-s3`; `apps/seerr/deployment-default-seerr.yaml` mounts `BucketInfo` for the active restore/replicate path |
