...
| `overseerr-litestream` | Active rollback source | COSI `BucketAccess/default/overseerr-litestream` creates `overseerr-litestream-s3`; no steady-state pod consumes it after the PostgreSQL cutover, but Seerr rollback procedures in `docs/seerr-postgres-migration.md` and `docs/litestream-recovery.md` still depend on it |
| `seerr-litestream` | Migration/rollback boundary | COSI `BucketAccess/default/seerr-litestream` creates `seerr-litestream-s3`; no steady-state pod consumes it after the PostgreSQL cutover, but Seerr migration and rollback procedures in `docs/seerr-postgres-migration.md` and `docs/litestream-recovery.md` still depend on it |
