...
For the initial sync, the script first creates verified SQLite online backups on Hestia in a bounded host-side step using a fresh mode-0700 directory under `/var/tmp`. It also writes a minimal manifest containing only the absent/present and path-type state of every allowlisted top-level database, profile, and profile database; it contains no database contents or secrets. The migration pod mounts that exact staging directory read-only and, before any target mutation, requires the manifest's exact schema and membership, rechecks every source path's type/state, rejects missing or extra staged backups, and verifies every present staged database is self-contained and passes `quick_check`. The pod therefore never opens live WAL databases through its read-only source mount and cannot silently treat a staged profile database as absent if its source profile drifts after staging. The stopped-source final sync still copies the exact database/WAL/SHM files.

Migration-pod and SQLite-staging cleanup is armed before staging and runs on success, failure, or signal. Cleanup attempts both resources even when one fails. It retains the migration-pod handle and returns failure unless a follow-up Kubernetes read authoritatively confirms that pod is absent, so a successful sync cannot disarm its traps while a root migration pod may still hold the target PVC writable.
