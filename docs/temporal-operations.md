...
`CiliumNodeConfig` changes require agent recreation before they take effect. After Flux has reconciled `platform` at the accepted revision, an authorized platform operator rolls one node at a time, waiting for the replacement agent to become Ready before continuing. Keep the ADD worker disabled and do not start workflows during this activation gate. For each node, verify inside its replacement agent that `enable-node-selector-labels` is `true` and `node-labels` is exactly `kubernetes.io/hostname`; after all three agents are replaced, require the Temporal Cilium policy status to be valid before testing Hestia TCP 7233 with its dedicated mTLS identity. Do not patch `cilium-config` or use `cilium config set`; those imperative values are not the source of truth.

Rollback is fail-closed: revert the source revision containing both this prerequisite and its dependent policy/order contract, let Flux remove the `CiliumNodeConfig`, then recreate one Cilium agent at a time and verify the previous effective configuration. Removing or disabling the prerequisite while the `fromNodes` policy remains present intentionally makes that policy invalid and leaves Temporal unavailable; never restore availability by widening ingress to `host`, `world`, a LAN CIDR, or a public service.
