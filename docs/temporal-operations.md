...
The Flux-managed `kube-system/ConfigMap/cilium-config` is the authoritative prerequisite for policy validation and agent defaults. Flux owns only `enable-node-selector-labels=true` and `node-labels=kubernetes.io/hostname` through non-pruning SSA merge, leaving unrelated Cilium configuration under its existing owner and preventing rollback from deleting the shared ConfigMap. The matching `kube-system/CiliumNodeConfig/node-selector-labels` supplies the same values to every agent during the serial activation rollout. Keeping the global and per-node sources identical is required: Cilium's operator validates `fromNodes` against the global daemon configuration, while agents consume their effective node configuration. The setting changes the shared `remote-node` identity into one hostname-scoped identity per node; entity-based policies remain valid, while label-based node policies can distinguish Hestia. The hostname-only filter avoids allocating identities for unrelated or frequently changing node labels. Cilium documents approximately 1 MB of additional agent memory per 1,000 nodes, so the three-node impact is negligible, but the identity change is cluster-wide and must be rolled out serially.

Temporal egress selects CoreDNS by its `coredns` Kubernetes service-account identity in `kube-system`, permits only TCP/UDP 53, and limits DNS queries to `*.*.svc.cluster.local`. Cilium wildcards do not cross DNS label separators, so this admits the Kubernetes service form `<service>.<namespace>.svc.cluster.local` while excluding external names and other cluster-local record shapes. Do not shorten the pattern to the ineffective `*.cluster.local`, replace the identity with a namespace-only selector, or grant general DNS/network egress.
...
`CiliumNodeConfig` changes require agent recreation before they take effect, and the Cilium operator must restart after the global ConfigMap reconciles so its policy validator reads the same enabled prerequisite. After Flux has reconciled `platform` at the accepted revision, an authorized platform operator first recreates the operator and requires it to become Ready, then rolls one agent at a time, waiting for the replacement agent to become Ready before continuing. Keep the ADD worker disabled and do not start workflows during this activation gate. Verify the operator no longer reports the `FromNodes/ToNodes` prerequisite error. For each node, verify inside its replacement agent that `enable-node-selector-labels` is `true` and `node-labels` is exactly `kubernetes.io/hostname`; after all three agents are replaced, require the Temporal Cilium policy status to be `Valid=True`. Then use a restricted Temporal-labeled disposable pod to prove both `temporal-frontend.temporal.svc.cluster.local` and `kubernetes.default.svc.cluster.local` resolve, prove an external name remains denied, delete the probe, and only then test Hestia TCP 7233 with its dedicated mTLS identity. Do not imperatively patch `cilium-config` or use `cilium config set`; Git is the source of truth.

Rollback is fail-closed and source-driven. In one reviewed source change, remove the dependent `fromNodes` contract or intentionally accept Temporal unavailability, set the merged global keys back to their recorded previous values, remove the `CiliumNodeConfig`, and update the validator with that reverted contract. Do not merely remove `cilium-config.yaml`: prune protection deliberately preserves the shared ConfigMap and its last managed values. After Flux reconciles the rollback, restart the operator, then recreate one Cilium agent at a time and verify the previous effective configuration. Removing or disabling either prerequisite while the `fromNodes` policy remains present intentionally makes that policy invalid and leaves Temporal unavailable; never restore availability by widening ingress to `host`, `world`, a LAN CIDR, or a public service.
