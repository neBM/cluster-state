...
| `victoriametrics` | Retained, unused | `Bucket/victoriametrics`, `BucketClaim/default/victoriametrics`, `BucketAccess/default/victoriametrics` | No current workload consumer; `Deployment/victoriametrics` uses local `victoriametrics-data` retention only | Backups were retired on 2026-06-09 after S3 listing metrics showed vmbackup inventory pressure on SeaweedFS filer metadata |
