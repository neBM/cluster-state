...
- SeaweedFS S3 is the live object-storage endpoint. Some workloads still use legacy `MINIO_*` secret key names for compatibility, and there is no active External Secrets controller in the cluster; see `docs/seaweedfs-s3-identities.md` for current mappings and manual rotation/repair steps, and `docs/seaweedfs-bucket-audit.md` for `/buckets` audit and cleanup rules. Scheduled restic data protection uses explicit restore-critical PVC mounts plus separate repository maintenance; the legacy whole-root `/buckets` restic CronJob/PV/PVC has been removed.
...
- Temporal is reconciled through the ordered `temporal-schema-setup`, `temporal-schema-upgrade`, and `temporal` Flux Kustomizations. Its runtime identity and 128 history shards are immutable after first boot; see `docs/temporal-operations.md` before schema work or restore.
