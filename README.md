...
- `apps/mail/deployment-default-rspamd.yaml` treats `/var/lib/rspamd` as node-local runtime state. Bayes data is already backed by Redis, so keep `rspamd` off SeaweedFS RWX storage and use `local-path-retain` for that mount.
