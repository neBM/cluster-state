...
- [x] T007 Copy `wildcard-brmartin-tls` secret from the `traefik` namespace to the `default` namespace as `mail-tls` using `kubectl get secret wildcard-brmartin-tls -n traefik -o yaml | sed 's/namespace: traefik/namespace: default/' | kubectl apply -f -`; verify with `kubectl get secret mail-tls -n default`
